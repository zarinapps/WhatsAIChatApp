...
class WithdrawHistoryResponseModel {
  WithdrawHistoryResponseModel({String? remark, String? status, List<String>? message, MainData? data}) {
    _remark = remark;
    _status = status;
    _message = message;
    _data = data;
      ...
      try {
        if (json['withdraw_information'] is List<dynamic>) {
          ...
          List<Details>? list = map.entries
              .map((e) => Details(name: e.value['name'], type: e.value['type'], value: e.value['value']))
              .toList();
          if (list.isNotEmpty) {
